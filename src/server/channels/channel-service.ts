...
export async function createChannel(ownerId:string,input:ChannelInput,deps=dependencies()){
  ...
  const {data,error}=await deps.db.from('live_channels').insert({owner_id:ownerId,slug:value.slug,title:value.title,description:value.description,category:value.category,thumbnail_url:value.thumbnailUrl,visibility:value.visibility,mux_live_stream_id:stream.id,mux_playback_id:playbackId,mux_stream_status:'offline',is_enabled:true}).select().single()
...
export async function getStreamCredentials(ownerId:string,recentAuth:boolean){if(!recentAuth)throw new Response('Recent authentication required',{status:403});const channel=await getOwnerChannel(ownerId);if(!channel)throw new Response('Channel not found',{status:404});const stream=await getMuxLiveStream(channel.mux_live_stream_id);return {serverUrl:RTMPS_SERVER_URL,streamKey:stream.stream_key,status:channel.mux_stream_status}}
export async function regenerateStreamKey(ownerId:string,recentAuth:boolean){if(!recentAuth)throw new Response('Recent authentication required',{status:403});const channel=await getOwnerChannel(ownerId);if(!channel)throw new Response('Channel not found',{status:404});const stream=await resetMuxStreamKey(channel.mux_live_stream_id);return {streamKey:stream.stream_key}}
export async function enableChannel(ownerId:string,enabled:boolean){const channel=await getOwnerChannel(ownerId);if(!channel)throw new Response('Channel not found',{status:404});await setMuxStreamEnabled(channel.mux_live_stream_id,enabled);await adminDb().from('live_channels').update({is_enabled:enabled,mux_stream_status:enabled?'offline':'disabled'}).eq('id',channel.id)}
export async function listDirectory(){const db=adminDb();const {data:channels}=await db.from('live_channels').select('*').eq('visibility','public').eq('is_enabled',true).order('updated_at',{ascending:false});const {data:replays}=await db.from('broadcasts').select('*').eq('is_replay_public',true).eq('status','ready').order('ended_at',{ascending:false}).limit(12);return {live:(channels??[]).filter((c:{mux_stream_status:string})=>c.mux_stream_status==='live'),replays:replays??[]}}
